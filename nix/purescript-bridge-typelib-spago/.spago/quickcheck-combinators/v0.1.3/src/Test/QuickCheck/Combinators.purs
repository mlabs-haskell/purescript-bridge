...
derive instance genericAtLeast ::
  ( Generic (t a) ta
  ) =>
  Generic (AtLeast n t a) _

instance arbitraryAtLeast ::
  ( Arbitrary a
  , Nat n
  , Unfoldable t
  , Traversable t
  ) =>
  Arbitrary (AtLeast n t a) where
...
instance arbitraryAtMost ::
  ( Arbitrary a
  , Nat n
  , Unfoldable t
  , Traversable t
  ) =>
  Arbitrary (AtMost n t a) where
  arbitrary = do
    let n = toInt' (Proxy :: Proxy n)
    l <- chooseInt 0 n
...
derive instance genericBetween ::
  ( Generic (t a) ta
  ) =>
  Generic (Between n m t a) _

instance arbitraryBetween ::
  ( Arbitrary a
  , Nat n
  , Nat m
  , Unfoldable t
  ...
  Arbitrary (Between n m t a) where
  arbitrary = do
    let
