...
extremelyUnsafeMkSumType = case mkSumType @t of
  SumType tInfo constructors instances -> SumType tInfo constructors (instances <> (Generic : GenericShow : HasConstrIndex : ToData : FromData : Json :  maybeToList (nootype . map snd $ constructors)))
...
mkSumTypeIndexed_ ::
  ...
  SumType 'Haskell
mkSumTypeIndexed_ f = SumType (mkTypeInfo @t) constructors (Generic : GenericShow : HasConstrIndex : ToData : FromData : Json :  maybeToList (nootype . map snd $ constructors))
  where
